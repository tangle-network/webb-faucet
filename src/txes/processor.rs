...
impl TransactionProcessingSystem {
            ...
            while let Some(transaction) = self.rx_receiver.recv().await {
                match transaction {
                    Transaction::Evm {
                        ...
                        self.handle_evm_tx(provider, to, amount, token_address, result_sender)
                            .await;
                    }
                    Transaction::Substrate {
                        ...
                        self.handle_substrate_tx(api, to, amount, asset_id, signer, result_sender)
                            .await;
                    }
                }
            }
    ...
    async fn handle_evm_tx<M: Middleware>(
        ...
        match token_address {
            Some(token_address) => {
                self.handle_evm_token_tx(provider, to, amount, token_address, result_sender)
                    .await
            }
            None => {
                self.handle_evm_eth_tx(provider, to, amount, result_sender)
                    .await
            }
        }
    }

    async fn handle_evm_eth_tx<M: Middleware>(
        &self,
        provider: M,
        to: Address,
        amount: U256,
    ...
    async fn handle_substrate_tx(
        ...
        match asset_id {
            Some(asset_id) => {
                self.handle_substrate_token_tx(api, to, amount, asset_id, signer, result_sender)
                    .await
            }
            None => {
                self.handle_substrate_native_tx(api, to, amount, signer, result_sender)
                    .await
            }
        }
    }
